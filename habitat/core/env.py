...
class Env:
    ...
    def reset(self) -> Observations:
        ...
        self.objID = []   ##list of object IDs inserted which we can use to remove on successful found
        ...
        for i in range(len(self.current_episode.goals)):
            ...
            self.objID.append(ind)
            if dataset_index == 6:
                self._sim._sim.set_translation(np.array(self.current_episode.goals[i].position) + np.array([0,0.3,0]), ind)
            else:
    ...
    def step(
        ...
        # else:
        #     self.currMap[currPix[0], currPix[1], 2] = 2  ##VIsited multiple times
        patch = self.currMap[currPix[0]-40:currPix[0]+40, currPix[1]-40:currPix[1]+40,:]
        patch = ndimage.interpolation.rotate(patch, -(observations["heading"][0] * 180/np.pi) + 90, order=0, reshape=False)
