...
@registry.register_sensor(name="MapSensor")
class AgentMapSensor(Sensor):
    def __init__(self, sim, config, **kwargs: Any):
        super().__init__(config=config)
        self._sim = sim
...
class RawMetrics(Measure):
    ...
    def reset_metric(self, *args: Any, episode, task, **kwargs: Any):
        ...
        self.goal_loc = [i.position for i in episode.goals]
        self.goal_obj = [i.object_category for i in episode.goals]
        self.update_metric(*args, episode=episode, task=task, **kwargs)
        ##
...
@registry.register_measure
class TrajMetrics(Measure):
    """All the raw metrics we might need
    """
    cls_uuid: str = "traj_metrics"
    ...
    def reset_metric(self, *args: Any, episode, task, **kwargs: Any):
        ...
        self._metric = None
        task.measurements.check_measure_dependencies(
            self.uuid, [SeenMap.cls_uuid, EpisodeLength.cls_uuid, DistanceToMultiGoal.cls_uuid, DistanceToCurrGoal.cls_uuid, SubSuccess.cls_uuid]
        )

        self._shortest_path_points = []


        self._shortest_path_points.append(
            [
                x.tolist() for x in self._sim.get_straight_shortest_path_points(
        ...
        for i in range(0, len(episode.goals)-1):
            self._shortest_path_points.append(
                [
                    x.tolist() for x in self._sim.get_straight_shortest_path_points(
                        episode.goals[i].position, episode.goals[i+1].position
                    )
                ]
            )
        

        self._shortest_path_points_grid = []

        for i in self._shortest_path_points:
            self._shortest_path_points_grid.append(
                [
                    self.conv_grid(
                        p[0],
                        p[2]
        ...
        self.discovered_goals_list = []
        self.goal_map_loc = []
        for g in episode.goals:
            self.goal_map_loc.append(self.conv_grid(g.position[0], g.position[2]))

       
        self.update_metric(*args, episode=episode, action = {'action': -1}, task=task, **kwargs)
        ##
    ...
    def update_metric(self, *args: Any, episode, action, task: EmbodiedTask, **kwargs: Any):
        ...
        self.act.append(action['action'])
        self.coord.append(current_position)
        self.orient.append(self._sim.get_agent_state().rotation.tolist().components.tolist())
        self.path_dist.append(self._agent_episode_distance)
        self.currgoal_dist.append(distance_to_curr_subgoal)
        self.currgoal_ind.append(task.currGoalIndex)
        self.sub_success.append(ep_sub_success)
        self.grid_coord.append(self.conv_grid(current_position[0], current_position[2]))

        discovered_goals = []
        observed_area = task.measurements.measures[SeenMap.cls_uuid].get_metric()
        for i in range(0, len(self.goal_map_loc)):
            if observed_area[self.goal_map_loc[i][0], self.goal_map_loc[i][0]] != 0:
                discovered_goals.append(i)
        self.discovered_goals_list.append(discovered_goals)
        ...
        self._metric = {
            ...
            'currgoal_dist': self.currgoal_dist,
            'currgoal_ind': self.currgoal_ind,
            'sub_success': self.sub_success,
            'grid_coord': self.grid_coord,
            'shortest_points': self._shortest_path_points,
            'shortest_grid_points': self._shortest_path_points_grid,
            'discovered_goals_list': self.discovered_goals_list,
        }


@registry.register_measure
class SeenMap(Measure):
    r"""SEEN map measure: same as FOW MAP
    """
    cls_uuid: str = "seen_map"
    ...
    def reset_metric(self, *args: Any, episode, task, **kwargs: Any):
        self._metric = None
        self._top_down_map = task.occMap
...
#     SoftSPL = objects_found/total_objects + 
#     (1/total_objects) * (distance_to_next_target/distance_from_prev_target_to_present)
#     """
...
class TopDownMap(Measure):
    ...
    def __init__(
        ...
        self.line_thickness = int(
            np.round(self._map_resolution[0] * 1 / MAP_THICKNESS_SCALAR)
        )
        self.point_padding = 3 * int(
    ...
    def _draw_point(self, position, point_type, inner_square):
        ...
        if inner_square==False:
            ...
            self._top_down_map[
                t_x - round(self.point_padding * 1 / 3) : t_x + round(self.point_padding * 1 / 3) + 1,
                t_y - round(self.point_padding * 1 / 3) : t_y + round(self.point_padding * 1 / 3) + 1,
            ] = maps.MAP_SOURCE_POINT_INSIDE_INDICATOR

    def _draw_circle(self, position, point_type):
        t_x, t_y = maps.to_grid(
            position[0],
            position[2],
            self._coordinate_min,
    ...
    def _draw_goals_positions(self, episode):
        if self._config.DRAW_GOAL_POSITIONS:
            ...
            for goal in episode.goals:
                try:
                    goal_color = goal.object_category.split('_')[1]
                    if goal_color == 'red':
                        self._draw_circle(
                            goal.position, maps.MAP_TARGET_POINT_INDICATOR_RED
                        )
                    elif goal_color == 'green':
                        self._draw_circle(
                            goal.position, maps.MAP_TARGET_POINT_INDICATOR_GREEN
                        )
                    elif goal_color == 'blue':
                        self._draw_circle(
                            goal.position, maps.MAP_TARGET_POINT_INDICATOR_BLUE
                        )
                    elif goal_color == 'yellow':
                        self._draw_circle(
                            goal.position, maps.MAP_TARGET_POINT_INDICATOR_YELLOW
                        )
                    elif goal_color == 'white':
                        self._draw_circle(
                            goal.position, maps.MAP_TARGET_POINT_INDICATOR_WHITE
                        )
                    elif goal_color == 'pink':
                        self._draw_circle(
                            goal.position, maps.MAP_TARGET_POINT_INDICATOR_PINK
                        )
                    elif goal_color == 'black':
                        self._draw_circle(
                            goal.position, maps.MAP_TARGET_POINT_INDICATOR_BLACK
                        )
                    elif goal_color == 'cyan':
                        self._draw_circle(
                            goal.position, maps.MAP_TARGET_POINT_INDICATOR_CYAN
                        )
                except AttributeError:
                    pass
    ...
    def _draw_shortest_path(
        ...
        if self._config.DRAW_SHORTEST_PATH:
            ...
            for i in range(0, len(episode.goals)-1):
                self._shortest_path_points.extend(
    ...
    def reset_metric(self, *args: Any, episode, **kwargs: Any):
        ...
        if self._config.DRAW_SOURCE:
            ...
            # )
            self._draw_point(
                episode.start_position, maps.MAP_SOURCE_POINT_INDICATOR, False
            )
            self._draw_point(
                episode.start_position, maps.MAP_SOURCE_POINT_INDICATOR, True
            )
    ...
    def update_metric(self, episode, action, *args: Any, **kwargs: Any):
        self._step_count += 1
        self.pathLength = kwargs["task"].measurements.measures[
            RawMetrics.cls_uuid
        ].get_metric()["agent_path_length"]
